...
package storage::huawei::oceanstor::restapi::mode::components::fan;
...
sub check {
    ...
    foreach my $entry (@{$self->{subsystems}->{fan}}) {
        ...
        $self->{output}->output_add(
            long_msg => sprintf(
                "fan instance '%s' status is '%s' [location: %s, running status: %s]",
                $instance,
        ...
        my $exit = $self->get_severity(label => 'default', section => 'fan', name => $name, value => $health_status->{$entry->{'HEALTHSTATUS'}});
