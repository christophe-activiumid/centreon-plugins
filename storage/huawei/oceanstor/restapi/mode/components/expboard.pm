...
package storage::huawei::oceanstor::restapi::mode::components::expboard;
...
sub check {
    ...
    $self->{output}->output_add(long_msg => 'checking expansion board');
    $self->{components}->{expboard} = { name => 'expboard', total => 0, skip => 0 };
    return if ($self->check_filter(section => 'expboard'));

    foreach my $entry (@{$self->{subsystems}->{expboard}}) {
        ...
        $self->{output}->output_add(
            long_msg => sprintf(
                "expansion board instance '%s' status is '%s' [model: %s, location: %s, running status: %s]",
                $instance,
        ...
        my $exit = $self->get_severity(label => 'default', section => 'expboard', name => $name, value => $health_status->{$entry->{'HEALTHSTATUS'}});
