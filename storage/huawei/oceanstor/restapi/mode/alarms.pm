...
package storage::huawei::oceanstor::restapi::mode::alarms;
#use Data::Dumper;
...
sub set_counters {
    ...
    $self->{maps_counters}->{space} = [
         { label => 'space-usage', nlabel => 'system.space.usage.bytes', set => {
...
sub manage_selection {
    ...
    my $result = $options{custom}->request(endpoint => '/alarms');
    my $data = $result->{data};
